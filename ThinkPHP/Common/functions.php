...
function in_array_case($value,$array){
 ...
 function sendEamil($toMail, $title, $content, $Attachment = '')
{
    $respon = array(
        'code' => 0,
        'msg' => ''
    );
    //require_once (app_path()."/Library/EMAIL/PHPMailer.php");
    //require_once (app_path()."/Library/EMAIL/Smtp.php");
    ...
    $mail->Username = 'bosexchange@g-town.com.cn'; // SMTP服务器用户名
    $mail->Password = 'Gtown123'; // SMTP服务器密码
    $mail->SetFrom($mail->Username, 'noreply'); // 设置发件人地址和名称
    ...
    $mail->addCC('eddy.chen@g-town.com.cn');
    //$mail->addCC('abyss@g-town.com.cn');
    //$mail->addCC('Cindy.wang@g-town.com.cn');
    // $mail->addCC('makaveli@g-town.com.cn');
    //$mail->addCC('sue@g-town.com.cn');
    //$mail->addCC('dem@g-town.com.cn');
    //$mail->addCC('rick.shen@g-town.com.cn');
    //$mail->addCC('zoe.hu@g-town.com.cn');
    if ($Attachment)
        $mail->AddAttachment($Attachment); // 附件
    if (! $mail->Send()) {
        //Log::useFiles(storage_path().'/logs/boc365.log');
        //Log::error('-邮件转发失败' . $mail->ErrorInfo );
        $respon['code'] = - 1;
