...
class CouponModel extends Model {
    ...
    function getList($model, $map, $sort = '') {
       ...
       // $count_model = clone $model;
        //取得满足条件的记录数
        $count = $model -> join('left join vrc_goods AS g ON c.goods_id = g.id') -> where($map) -> count();
        ...
        if ($count > 0) {
            ...
            //分页查询数据
            $vo_list = $model -> join('left join vrc_goods AS g ON c.goods_id = g.id')->field('c.*,g.name')->where($map) ->order($sort) -> limit($p -> firstRow . ',' . $p -> listRows) -> select();
           // echo $model->getLastSql();exit;
